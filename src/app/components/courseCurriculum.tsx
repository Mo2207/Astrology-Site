...
import dots from "../../../public/assets/img/curriculum/icons/scrolling-dots.svg";
...
export default function CourseCurriculum() {
  ...
  return (
    <section id="curriculum" className="pt-12 bg-[#FFFFFF] text-gray-900 h-[25rem]">
      ...
      <div className="relative w-full overflow-hidden justify-center flex">
        
      {/* <Image
          onClick={scrollLeft}
          src={left}
          alt="left arrow icon"
          className="w-[3rem] h-[3rem] absolute left-5 bottom-0 -translate-x-y-1/2 sm:hidden hover:scale-150 transition delay-100 duration-300 ease-in-out"
        />
        <Image
          onClick={scrollRight}
          src={right}
          alt="right arrow icon"
          className="w-[3rem] h-[3rem] absolute right-5 bottom-0 -translate-x-y-1/2 sm:hidden hover:scale-150 transition delay-100 duration-300 ease-in-out"
        /> */}
        ...
        > 
          {Object.keys(curriculum).map((key) => (
            <div
              key={key}
              className="flex-shrink-0 flex flex-col justify-start w-full h-[90%] sm:w-[20rem] min-w-0 bg-[#faf5f3] shadow-md rounded-xl p-6 snap-center border border-[#AD6049]"
            >
